...
**Load:** `roles/content-designer/rules/content.md`, the project glossary, and the sector entry, tone is one
of the five convention axes and it is not yours to pick.

**Load first, in this order:**

1. `roles/content-designer/rules/content.md`: every state written, glossary-bound, length-realistic
...
When phase 5 runs, you produce three points for **content** and nothing else. There is no estimation rule to load: the `estimate` role and its
...
```json
"estimate": { "content": { "o": 0, "m": 0, "p": 0, "by": "pica-content-designer" } }
```
