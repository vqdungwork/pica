...
**Load:** `roles/ux-engineer/rules/html-prototype.md`, `roles/ux-engineer/rules/html-gates.md`,
`roles/design-researcher/rules/design-vocabulary.md`, and `native-mobile.md` if a native target ships. Then the
sector entry **in full**.

**Load first, in this order:**

1. `roles/ux-engineer/rules/html-prototype.md`: layout, the flow, states, real assets
2. `roles/ux-engineer/rules/html-gates.md`: what has to measure zero before anyone sees it
3. `roles/ux-engineer/rules/native-mobile.md`: when a viewport is a device rather than a width
4. `roles/product-manager/rules/proposals.md`: S1 to S5 are offers, not decisions
...
When phase 5 runs, you produce three points for **design** and nothing else. There is no estimation rule to load: the `estimate` role and its
...
```json
"estimate": { "design": { "o": 0, "m": 0, "p": 0, "by": "pica-ux-engineer" } }
```
