...
1. `roles/ux-researcher/rules/discovery.md`: classes, frequency, the people who can veto
2. `roles/business-analyst/scripts/industry-check.mjs --show <sector>`: every stakeholder the sector
...
```json
"estimate": { "discovery": { "o": 0, "m": 0, "p": 0, "by": "pica-ux-researcher" } }
```
