...
Load `roles/product-manager/rules/intake.md` before anything else. It is core's own rule now, so intake runs
...
**Three things intake no longer asks.** The audience is researched by `pica-ux-researcher`, because most
...
**`pica-core` cannot depend on `pica-ui-designer`**: research depends on core, and a cycle is not
