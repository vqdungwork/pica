...
These rules lived in `roles/design-researcher/rules/research.md` until 2.0.0, which meant intake could not
...
"refuse to invent and call it reuse" in `roles/design-researcher/rules/research.md` already forbids the way out
of that.
...
Resolves against `roles/business-analyst/data/industries.json`. **An unresolvable field is refused, not
...
`roles/business-analyst/data/archetypes.json`.
...
| The audience | **researched, not declared.** `pica-ux-researcher` profiles it: most briefs cannot answer it, and a researched answer beats a guessed one |
