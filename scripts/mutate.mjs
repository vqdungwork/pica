...
function makeFixture() {
  ...
  const state = {
    ...
    discovery: {
      ...
      stakeholders: [
        { role: "compliance officer", wants: "to prove afterwards who did what",
          fears: "an action with no attributable actor", decides: true, vetoes: true,
          wouldBlockIf: "the audit trail cannot be exported in the inspector's format" },
        { role: "fraud analyst", wants: "to stop a payment mid-flight",
          fears: "a queue with no age on it", decides: true, vetoes: false,
          wouldBlockIf: "" },
        { role: "head of branch operations", wants: "approvals to take less of the day",
          fears: "a second system to keep open", decides: false, vetoes: true,
          wouldBlockIf: "approvers have to leave the core banking screen to use it" },
      ],
      competitors: [
...
const M = [
  ...
  ["stakeholder-fears",  "discover/scripts/discover-check.mjs", [S], "discovery", (s) => delete s.discovery.stakeholders[0].fears],
