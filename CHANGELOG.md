...
3.0.0 was tagged, released and announced on a commit whose CI run failed. Three fixes landed on
`main` in the hour that followed and none of them was in the tag, so the artefact people could
download was the broken one for as long as it was the latest release.

What they got:

- **`pica-ux-researcher` would not install.** Its manifest declared `"agents": ["./agents"]`, and
  Claude Code rejects that field with `agents.0: Invalid input`. Eleven of twelve roles installed
  and the twelfth did not, which is a worse failure than none installing, because the workflow
  looks present until it reaches discovery.
- **The mutation suite was dead.** `mutate.mjs` still named the `packages/` directory 3.0.0 had
  removed, so the suite that proves every other check exits before it proves anything.
- **`knowledge-gen` knew seven agents of twelve**, and the register embedded in five of them was
  therefore generated by something that did not know they existed.

3.0.1 is 3.0.0 with those three already in it, plus the check below. There is no new behaviour and
no new role. If 3.0.0 installed cleanly for you, nothing here changes what it does.

### The shape the installer requires

Every one of those three defects was caught by CI. The first was not — it was found by a person
installing twelve plugins by hand, and the reason is worth writing down.

`validate-packages` asked whether a declared agent file existed. `./agents` is a directory that
exists, so it passed. The installer asks something else: agents are `.md` **files**, and directories
are not accepted. A manifest could satisfy every check in this repository and still be uninstallable,
and one did.

Both halves are now held:

- `validate-packages` checks every component path in every `plugin.json` against the documented
  rules — agents are files, skills are directories, commands are a `.md` file or a directory, and
  every path must exist inside the plugin root. It names the same coordinate the installer does,
  `agents.0`, and says what to write instead.
- CI runs `claude plugin validate --strict` over all fourteen manifests. That command **is** the
  installer's schema rather than this repository's reading of it, and it knows fields we have not
  thought to check. Where the two disagree, it is right.

The node check earns its place by needing no install. The official one earns its place by being the
authority. Neither is redundant.

### Still true, and still the cause

A release was tagged before its pipeline was green. No check prevents that today, and none of the
above would have. It is the next thing worth fixing.
