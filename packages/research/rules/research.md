...
Six inputs. **Refuse to start without all of them**, and say which are missing rather than proceeding
on assumptions.
...
Closeout re-reads it cold and is forbidden from reading the contract instead, because copies drift. A
brief that lives only in a conversation is gone by the time that step runs on any project that spans
more than one sitting. Until it has a path, "keep the original wording" is a preference: nothing names
where it went, so nothing can check it is still there.
...
### 6. The field, the audience, and the conditions of use

Three facts, none of which the brief reliably states, and all of which a human answers in ten seconds.

- **The field**, named narrowly. "Retail banking dashboard", not "fintech". "Clinic intake on a shared
  tablet", not "healthcare". The narrow name has real products behind it that can be looked at; the broad
  one has only a mood.
- **The audience**, and how often they use it. A tool someone lives in for six hours a day and a tool
  someone opens twice a year are opposite designs, and nothing in a screenshot distinguishes them.
- **The conditions of use.** At a desk, outdoors, one-handed, wearing gloves, in a noisy room, on a
  shared device. This is the input nobody asks for and the one that most often invalidates a design
  after it is built. The same clinical product is a different product in a lit consulting room and in a
  moving ambulance.

These feed the direction (below). Without them a direction can only be a preference.
