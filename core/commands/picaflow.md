...
| Phase | Agent | Loads |
|:--|:--|:--|
| discover | `pica-ux-researcher` ×3–5, one per segment | `discovery.md` |
| research | `pica-design-researcher` ×3–5, one per product, none seeing another's findings | `design-vocabulary.md` |
| analyse A, C | `pica-business-analyst` | `business-analysis.md`, `domain-knowledge.md`, `industry-knowledge.md` |
| analyse B | `pica-systems-analyst` | `modelling.md` |
| structure | `pica-ux-designer` | `structure.md` |
| direction, foundations | `pica-ui-designer` | `direction.md`, `design-vocabulary.md` |
| architecture | `pica-solution-architect` | `architecture.md` |
| the build | `pica-ux-engineer` | `html-prototype.md`, `html-gates.md`, `react-demo.md`, `native-mobile.md` |
| the words | `pica-content-designer` | `content.md` |
...
| intake, scope, close | `pica-product-manager`, on the main thread | `intake.md`, `proposals.md` |
| the port | `pica-design-ops`, gated by hook | `figma-*.md` |
...
|---|---|
| `pica-ux-researcher` | no users, no pain with a frequency, no veto holders, **no audience profile**: every floor the design owes is unset |
| `pica-ui-designer` | nothing is measured, and the direction has no precedent behind it |
| `pica-business-analyst` | no PRD, no glossary, no sector, **no models**. The client has nothing to agree to |
| `pica-ux-designer` | no lo-fi gate: structure and visual design get approved together, which is the bundling the gate exists to prevent |
| `pica-solution-architect` | the stack is chosen by whoever scaffolds the demo, and the estimate prices an architecture nobody wrote down |
| `pica-ux-engineer` | nothing is built and nothing is measured |
| `pica-content-designer` | screens carry placeholder text |
| `pica-evaluator` | no independent evaluation before the client sees it |
...
| **2.1** | **Fan out** `pica-ux-researcher`, one per segment: pain with a frequency and a class, the people who can veto it, competitor pricing, the market derived | `discovery`, `stakeholders` |
...
| **3.1** | **Fan out** `pica-design-researcher`, nine foundations each, on products the archetype picks | `measured`, `tokens/` |
...
| Step | Does | Writes |
|---|---|---|
| **4.A** | `pica-business-analyst`: the real problem, glossary and data dictionary, business rules, domain constraints, **NFRs as numbers** | `problem`, `glossary`, `businessRules`, `nfr` |
| **4.B** | `pica-systems-analyst`: **TO-BE process model** (BPMN when it crosses roles), **domain model**, **CRUD matrix**, **state model** | `toBe`, `domainModel`, `rolesPermissions`, `stateModel` |
| **4.C** | `pica-business-analyst`: use cases, requirements **classified**, journey maps per laned actor, the delta, **the PRD** | `useCases`, `requirements`, `journeys`, `delta`, `docs/prd.md` |
| **4.D** | `pica-solution-architect`: the stack layer by layer with its argument, a mechanism per NFR, a contract per integration **including what happens when it is down**, environments with a purpose, what is **not** being built, data owner and retention per entity | `architecture`, `stack`, `environments` |
...
| Step | Does | Writes |
|---|---|---|
| **5.0** | `pica-ux-designer`: content inventory, IA, screen inventory traced to use cases, data source per screen. The **state matrix arrives from analysis** | `screens` |
| **5.1** | `pica-ux-designer`: **lo-fi structure**, greyscale, real content lengths, every state drawn or excused | `html/structure/` |
| **5.2** | `pica-ui-designer`: **offer the direction**, the busiest screen built three ways, three different traditions, one of them the sector's own, each naming its measured source. Then S2–S5 | `proposals` |
| **5.3** | `pica-ui-designer`: **foundations**, palette with roles and contrast proved before it is offered, type scale, spacing, the kit in every state, the icon set, motion tokens | `direction`, `tokens/`, `html/design-system.html` |
| **5.4** | `pica-ux-engineer`: every screen at every viewport in every state, built on the 5.3 tokens and the `pica-content-designer` strings | `html/` |
