...
file is the map. The rules now live with the package that owns them, under `roles/*/rules/`.
...
| # | Step | Command | Rules |
|---|---|---|---|
| 0 | Intake: brief verbatim, sources labelled, **analytics and support logs requested** | `/pica` | `roles/design-researcher/rules/research.md` |
| 1 | Research: analytics, support logs, journey map, **measure 3 to 5 shipped products** | inside `/pica` | `roles/design-researcher/rules/design-vocabulary.md` |
| 0.9 | Discovery: who uses it, **what actually hurts counted rather than assumed**, who can veto it and what they fear, what the field charges | `/pica-discover` | `roles/ux-researcher/rules/discovery.md` |
| 2 | Domain knowledge, glossary, AS-IS, TO-BE, **the delta**, business rules, use cases, domain model, PRD | `/pica-analyse` | `roles/business-analyst/rules/business-analysis.md`, `roles/business-analyst/rules/domain-knowledge.md`, `roles/business-analyst/rules/industry-knowledge.md` |
...
| # | Step | Command | Rules |
|---|---|---|---|
| 3.0 | IA, screen inventory traced to use cases, state matrix | inside `/pica-wp` | `roles/ux-engineer/rules/html-prototype.md` |
| 3.1 | **Design direction**, named against a tradition, asserted as numbers | inside `/pica` | `roles/design-researcher/rules/design-vocabulary.md` |
| 3.2 | Tokens in **three tiers**: primitive, semantic, component | inside `/pica` | `roles/design-researcher/rules/research.md` |
| 3.3 | UI kit in HTML, before any screen consumes it | inside `/pica` | `roles/ux-engineer/rules/html-prototype.md` |
| 3.4 | Build the package at every viewport, **every state**, plus the interactive flow | `/pica-wp <name>` | `roles/ux-engineer/rules/html-prototype.md` |
| 3.5 | **The words**, every state, bound to the glossary | `/pica-copy` | `roles/content-designer/rules/content.md` |
| 3.7 | **Measure**: verify-html, coverage-check, parity-check, flow-check, copy-check. All zero, or fix | inside `/pica-wp` | `roles/ux-engineer/rules/html-gates.md` |
| 3.8 | **3 to 5 independent evaluators**, then a walkthrough per use case | `/pica-evaluate` | `roles/evaluator/rules/evaluation.md` |
...
| 3.11 | **GATE: the client approves this package** | inside `/pica-wp` | `roles/ux-engineer/rules/html-gates.md` |
...
| # | Step | Command | Rules |
|---|---|---|---|
| 5f | Port to Figma, verify frame by frame, wire the prototype | `/pica-port`, `/pica-review`, `/pica-prototype` | `roles/design-ops/rules/figma-screens.md` |
...
Read `roles/design-ops/rules/figma-rebuild.md` before starting one. The short
...
|---|---|---|
| `pica-core` | : | the state schema, every gate, the check runner, and `/picaflow` |
| `pica-product-manager` | core | intake, the scope boundary and its exclusions, closeout, feedback triage |
| `pica-ux-researcher` | core | users and what hurts, the people who can veto it, competitor pricing, the market derived from sourced factors |
| `pica-business-analyst` | core | elicitation, domain knowledge, AS-IS and TO-BE, business rules, use cases, the PRD, **the problem stated as a number** |
| `pica-systems-analyst` | core, business-analyst | the four models: process, domain, state and permissions |
| `pica-solution-architect` | core, business-analyst, systems-analyst | the stack, a mechanism per NFR, integration contracts, environments |
| `pica-design-researcher` | core, product-manager | the source audit, the nine foundations, design vocabulary, token provenance |
| `pica-ux-designer` | core, systems-analyst | content inventory, IA, user flows with their failures, greyscale structure |
| `pica-ui-designer` | core, design-researcher | the visual direction, the palette proved before it is offered, style tiles |
| `pica-ux-engineer` | core, ui-designer, content-designer | every screen at every viewport, and the measured gate |
| `pica-content-designer` | core, business-analyst, systems-analyst | the words: every state written, bound to the glossary |
| `pica-evaluator` | core, ux-engineer | independent evaluators, cognitive walkthrough, build versus design |
| `pica-design-ops` | core, ux-engineer | the port, annotations, and the geometry diff |

`pica` installs all twelve. A project that will never touch Figma installs `pica-ux-engineer`, which pulls in
`pica-core`, `pica-ui-designer` and `pica-content-designer` and never sees the Figma half. A team that only wants the business
analysis installs `pica-business-analyst`, which pulls in `pica-core` and nothing else.
...
pica decides well and, until 0.9.1, decided almost everything. `roles/product-manager/rules/proposals.md` names
...
| Agent | Package | Step |
|---|---|---|
| `pica-product-manager` | product-manager | 1.1–1.5, and 7.1 |
| `pica-ux-researcher` | ux-researcher | 2.1–2.3, fanned out, one per segment |
| `pica-design-researcher` | design-researcher | 3.1, fanned out, none seeing another's findings |
| `pica-business-analyst` | business-analyst | 4.A, 4.C |
| `pica-systems-analyst` | systems-analyst | 4.B |
| `pica-solution-architect` | solution-architect | 4.D |
| `pica-ux-designer` | ux-designer | 5.0–5.1 |
| `pica-ui-designer` | ui-designer | 5.2–5.3 |
| `pica-ux-engineer` | ux-engineer | 5.4, 5.7 |
| `pica-content-designer` | content-designer | 5.4 |
| `pica-evaluator` | evaluator | 5.6, fanned out, **no write access** |
| `pica-design-ops` | design-ops | 7f |
...
- **Capture an appearance baseline before any bulk mutation.** `roles/design-ops/scripts/capture-baseline.js`, then diff
...
| Figma seat with enough MCP calls | any part of 7f | size the work to the budget, see `roles/design-ops/rules/figma-mcp.md` |
| playwright | the capture harness in `roles/ux-engineer/scripts/` | say the measured diff is unavailable, and do not silently fall back to eyeballing |
...
`roles/design-researcher/rules/research.md`: a copy rule stated in conversation lasts about a
...
|---|---|---|
| `problem-check.mjs` | the metric, its baseline, the guardrail, the counter-evidence, the HMW question, the trigger, the commercial constraint, the tiers, the freeze | **`grep -rl 'state.problem' roles/*/scripts` returned nothing.** The number the project is judged by, and that closeout reads back, had no check behind it |
...
- `roles/design-researcher/rules/research.md`: research package. Intake packet, contract, exclusions, audit breadth, token provenance, mock-data provenance, client copy rules, data ownership
- `roles/ux-engineer/rules/html-prototype.md`: html package. Layout, the tabbed review page, options versus the interactive flow, navigation state, the tall-screen pair, real assets, state matrices
- `roles/ux-engineer/rules/html-gates.md`: html package. The measured HTML gate, the flow gate, viewport parity, HTML-only coverage, behaviour review for prototypes, definition of done
- `roles/design-ops/rules/figma-elements.md`: figma package. Token layers including Border, geometry binding, alpha in tokens, numeric font weights, font-package forensics, component tiers, instance constraints, component granularity, fake variant axes, naming by role, merge mechanics
- `roles/design-ops/rules/figma-screens.md`: figma package. Frames, states, alignment and vertical centring, screen chrome pinning, CSS to auto-layout, the circle trap, API traps
- `roles/design-ops/rules/figma-mcp.md`: figma package. Rate limits and call budget, `page.loadAsync` for whole-file reads, write discipline
- `roles/design-ops/rules/figma-gates.md`: figma package. The Figma audit checklist, appearance baselines, geometry-diff tolerances, the deviations register, definition of done
- `roles/design-ops/rules/figma-rebuild.md`: figma package. Rebuilding a client's existing Figma file: the source as arbiter, the shared coordinate system, positional parity, baselining every lens against the source, deciding the source's own defects
